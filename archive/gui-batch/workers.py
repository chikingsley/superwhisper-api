"""Multi-worker controller for GUI-orchestrated Superwhisper transcription.

Spawns threads across multiple macOS user accounts (each with their own
Superwhisper app instance) to process audio in parallel via the GUI.
Requires root so the controller can read all workers' SQLite databases.
"""
...
from superwhisper_api.gui.cli import run_job
...
class Worker:
    """Configuration for a Superwhisper worker macOS user account."""
    ...
    @property
    def uid(self) -> int:
        """Unix user ID of the worker account."""
        return pwd.getpwnam(self.user).pw_uid

    @property
    def home(self) -> Path:
        """Home directory of the worker account."""
        return Path(pwd.getpwnam(self.user).pw_dir)

    @property
    def db(self) -> Path:
        """Path to the worker's Superwhisper SQLite database."""
...
def build_parser() -> argparse.ArgumentParser:
    """Build the argument parser for superwhisper-gui-workers."""
    parser = argparse.ArgumentParser(
        prog="superwhisper-gui-workers",
...
def as_worker_command(worker: Worker, command: list[str]) -> list[str]:
    """Wrap a command to run as a specific worker user."""
    return [
        "sudo",
        "launchctl",
        "asuser",
...
def output_paths(job_dir: Path, run_id: str, worker: Worker) -> tuple[Path, Path, Path, Path]:
    """Return input and output file paths for a worker job."""
    paths_file = job_dir / f"{run_id}_{worker.name}_paths.txt"
    results = job_dir / "results"
    jsonl = results / f"gui.{worker.name}.jsonl"
    failures = results / f"gui.{worker.name}.failures.jsonl"
...
def preflight_worker(worker: Worker, job_dir: Path, run_id: str) -> list[str]:
    """Verify a worker is ready to process audio."""
    errors: list[str] = []
...
def start_superwhisper(worker: Worker, dry_run: bool) -> None:
    """Launch the Superwhisper app for a worker."""
...
def open_audio_as_worker(worker: Worker, audio_path: Path) -> None:
    """Open an audio file in Superwhisper as a worker user."""
...
def run_worker(worker: Worker, job_dir: Path, run_id: str) -> int:
    """Run the transcription job for a single worker."""
...
def run_controller(selected: list[Worker], job_dir: Path, run_id: str, dry_run: bool) -> int:
    """Coordinate and run jobs across multiple workers."""
...
def main() -> int:
    """Entry point for the superwhisper-gui-workers CLI."""
    ...
    if os.geteuid() != 0:
        print(
            "superwhisper-gui-workers must run as root so one controller can read "
